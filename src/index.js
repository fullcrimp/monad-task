...
//         [1, 2], [1, -2], [-1, 2], [-1, -2]
//       ]
//     return posArr.reduce(function (result, pos) {
//         return result.concat(moves.map(function (shift) {
...
let getNextPosArr = (posArr) => posArr.reduce((result, pos) => {
  let shiftCombinations = [1, 2, -1, -2].reduce((acc, val1, _, arr) => [...acc, ...arr.filter((val2) => Math.abs(val2) !== Math.abs(val1) ).map((val3) => [val1, val3])], [])
